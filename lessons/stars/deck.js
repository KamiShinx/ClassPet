...
(function () {
  ...
  Ctl.future = (function () {
    const STEPS = [
      { n: 'לפני 4.6 מיליארד שנה',
        t: 'ענן ענק של גז ואבק התכווץ עד שהמרכז נדלק — וזאת השמש. מהשאריות שנשארו מסביב נבנו כוכבי הלכת, וגם כדור הארץ.' },
      { n: 'היום',
        t: 'השמש צהובה ורגועה, בערך באמצע החיים שלה. ארבעת כוכבי הלכת הקרובים מקיפים אותה — הקרובים יותר מסתובבים מהר יותר.' },
      { n: 'בעוד 5 מיליארד שנה',
        t: 'הדלק במרכז ייגמר והשמש תתנפח לענק אדום. היא תבלע קודם את כוכב חמה, אחר כך את נוגה, ותגיע כמעט עד המסלול שלנו. כדור הארץ יישרף.' },
      { n: 'בעוד 8 מיליארד שנה',
        t: 'השכבות החיצוניות יעופו החוצה ויתפזרו בחלל. במרכז יישאר ננס לבן — גחלת קטנטנה וזוהרת, בערך בגודל של כדור הארץ.' },
    ];
    const list = $('#fu-list'), title = $('#fu-title'), text = $('#fu-text');
    let cur = 1, seen = 0, done = false;
    const marked = {};
    STEPS.forEach((e, i) => {
      const b = document.createElement('button');
    ...
    // בלי יציאה מוקדמת: לחיצה חוזרת על אותו שבב מריצה את המעבר מחדש
    function pick(i) {
      cur = i;
      items.forEach((b, k) => b.classList.toggle('on', k === i));
      title.textContent = STEPS[i].n;
      text.textContent = STEPS[i].t;
      const s = Stage3D.inst('future');
      if (s) s.select(i);
      if (!marked[i]) {
        marked[i] = 1; seen++;
        if (seen >= STEPS.length && !done) { done = true; addScore(10); }
      }
    }
    return { enter() { pick(cur); } };
  })();
